...
function load_table_data(data, page) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        html += "<td>" + data[i].related_device_id_id + "</td>"
        html += "<td>" + data[i].label_cn + "</td>"
        html += "<td>" + data[i].dev_language + "</td>"
        html += "<td>" + data[i].middleware + "</td>"
...
$('.bk-tab2-head ul li').live("click", function () {
    ...
    if (tab_id == 'base') {
        ...
        var tab_url = "/cmdb/process/related/" + id + "/"
    }
    else if (tab_id == 'alarm') {
        var tab_url = "/cmdb/process/alarm/" + id + "/"
    }
    else if (tab_id == 'record') {
        var tab_url = "/cmdb/process/record/" + id + "/"
    }
    if (tab_url) {
        $.ajax({
            type: "POST",
